...
rfind -user "$UID_T_MAX" >/dev/null 2>err || fail=1
compare /dev/null err || fail=1

rfind -group "$GID_T_MAX" >/dev/null 2>err || fail=1
compare /dev/null err || fail=1
...
returns_ 1 rfind -user "$UID_T_OFLOW" -name enoent >/dev/null 2>err || fail=1
...
returns_ 1 rfind -group "$GID_T_OFLOW" -name enoent >/dev/null 2>err || fail=1
sed 's/^.*find/find/' err > err2 || framework_failure_
compare exp err2 || fail=1
